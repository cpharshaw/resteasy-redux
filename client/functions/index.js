...
const savePlaceEntry = locationID => {
  return admin.firestore().collection('reviews')
    ...
    .get()
    .then(responseData => {
      // const responseData = QueryDocumentSnapshot;
      functions.logger.info("Inside firestore get", responseData);

      const firestoreResponse = [];

      const createArr = async () => {
        return responseData.forEach(
          review => {
      ...
      const createReview = data => functions.logger.info("**async function, passed the data array", data);

      createArr()
        .then(
          createReview(firestoreResponse) //works!
        )
...
exports.reviewSubmitted = functions.firestore.document('reviews/{reviewID}').onCreate(
  (QueryDocumentSnapshot) => {
    ...
    //   "basicInfo": { "locationState": "MD", "locationCategory": "Pool", "outOfOrder": false, "locationCity": "College Park", "restroomUsed": "Men's", "timeOfVisit": "Morning", "locationZip": "20740", "locationCountry": "United States", "locationName": "Seven Springs Neighborhood Pool", "locationNotes": "" }, 
    //   "features": { "price": "", "babyStation": false, "admission": "Free", "genderNeutral": false, "cleaningSchedule": false, "accessible": false } 
    // }
    ...
    //   });
        ...
        // const placeReview = {
        //   placeID: doc.locationID,
        //   placeName: doc.locationName,
        //   admission: admission,
        //   priceUSD: priceUSD,
        //   genderNeutralOption: genderNeutralOption,
